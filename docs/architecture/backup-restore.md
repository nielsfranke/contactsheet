...
- **`app/services/restore_service.py`** — `restore(db, archive, password)`:
  ...
  5. **Regenerate missing renditions** from the restored originals
     ...
     Camera RAW is covered too: the regenerator opens sources through the same
     `_open_source` as the upload path, so RAW previews are rebuilt from the
     file's embedded JPEG.
